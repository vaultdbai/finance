...
def load(connection: duckdb.DuckDBPyConnection, symbol: str):
    ...
    try_transform_and_insert(
        connection, ticker, "capital_gains", symbol, ["symbol", "date"]
    ...
    try:
        try_transform_and_insert(
            connection,
            ticker.quarterly_income_stmt.transpose(),
    ...
    try:
        try_transform_and_insert(
            connection,
            ticker.balance_sheet.transpose(),
            "balance_sheet",
            symbol,
            [],
            "symbol",
    ...
    try:
        try_transform_and_insert(
            connection,
            ticker.quarterly_balance_sheet.transpose(),
    ...
    try:
        try_transform_and_insert(
            connection,
            ticker.quarterly_cashflow.transpose(),
            "quarterly_cashflow",
            symbol,
            [],
            "symbol",
